...
function createCustomField(customFieldName, storedData, customField) {
    ...
    if (customField) {
        ...
        // making the api call
        $.ajax(settings1).done(function (res) { console.log(res); });
    ...
    // if the custom field doesn't exist
        ...
        $.ajax(settings2).done(function (response) {
            ...
            $.ajax(settings3).done(function (res) { console.log(res) });
        });
...
$(document).ready(function () {
    ...
    if (url.indexOf("admin/plugins") > -1) {
        console.log("field exists", checkedStatusField);
        var checkedStatus = false;
        if (checkedStatusField) {
            checkedStatus = !!Number(checkedStatusField.Values[0]);
        }
        else {
            createCustomField("checkedstatus", "0", checkedStatusField);
        }
        console.log("status of the check", checkedStatus);
        var slider = document.getElementById("toggle-button");
        console.log("slider checkbox", slider.checked);
        slider.checked = checkedStatus;
        console.log("slider checkbox after assignment", slider.checked);
    }
